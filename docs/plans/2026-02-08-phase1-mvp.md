...
**Architecture:** Next.js 14 App Router with server components by default, client components only where interactivity is needed. Supabase handles auth, database, and storage. API routes call Anthropic Claude for chapter generation. Zustand for minimal client state.

**Tech Stack:** Next.js 14, TypeScript, Tailwind CSS, Supabase (Auth + PostgreSQL + Storage), Anthropic Claude API (claude-sonnet-4-20250514), Framer Motion, Zustand, React Hook Form + Zod
